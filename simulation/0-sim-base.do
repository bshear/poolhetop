...
local outfilename "ests-a25-n_$ncond-sd_$sdcond-$cutname-${repmin}_${repmax}.dta"
...
qui forv r = $repmin / $repmax {
	...
	foreach cutcons in free {
		
		use "`full'" , clear	
		keep if rep == `r'

		// pooled and trend models with all sets pooled
		...
		// do full hetop AND homop separately to aid convergence
		
		g rowid = _n
		
		qui levelsof grade , local(grade_levs)
		qui levelsof year  , local(year_levs)
		
		foreach g in `grade_levs' {
			foreach y in `year_levs' {
				...
				preserve
					...
					foreach m in fhet fhom {	
						...
						if _rc | e(converged) == 0 {
							* try2: no start values
							local try=`try'+1
							cap ``m'mod' iter(200)
						}

						if _rc | e(converged) == 0 {
							* try3: start values and difficult option
							local try=`try'+1
							cap ``m'mod' iter(200) start difficult
						}

						if _rc | e(converged) == 0 {
							* try4: no start values and difficult option
							local try=`try'+1
							cap ``m'mod' iter(200) difficult
						}
						...
						if !_rc & e(converged) == 1 {
							...
							g double icchat_`m' = r(icchat`lev') if id == 1
							g double icchat_`m'_se = r(icchat`lev'_var)^.5 if id == 1
					...
					tempfile tempg`g'y`y'
					save "`tempg`g'y`y''"
						
				restore
					
				merge 1:1 rowid using "`tempg`g'y`y''" , nogen update
				
			}
		}	// grades and years for hetop
